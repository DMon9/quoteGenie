...
(function (window) {
  ...
  `;
    ...
    function autoWireButtons() {
        const isMobileEnhanced = (location.pathname || '').includes('mobile-index-enhanced');
        ...
        allButtons.forEach((button, index) => {
            ...
            // Generate Quote buttons (redirect to mobile enhanced flow)
            else if (
                text.includes('Generate Quote') ||
                text.includes('Generate your first quote') ||
                (text.includes('Generate') && text.includes('Quote')) ||
                // Additional phrases users may click
                text.includes('Start Your Quote') ||
                (!isMobileEnhanced && text === 'Get Quote') ||
                text.includes('Generate AI Quote')
            ) {
                button.setAttribute('data-cta-wired', 'true');
                button.style.cursor = 'pointer';
                console.log(`[CTA Buttons] Wired generate quote button: "${text}"`);
                button.addEventListener('click', function (e) {
                    e.preventDefault();
                    // If we're already on the mobile enhanced page, preserve native behavior (e.g., scroll)
                    if (isMobileEnhanced) {
                        return;
                    }
                    window.location.href = 'mobile-index-enhanced.html';
                });
            }
        });
